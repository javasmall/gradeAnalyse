...
function allSubpt_getJson(){
    ...
    function getJson() {
        for(var i=0;i<5;i++) { //选定考试后循环获取5科的成绩
            var URL="http://www.biggsai.com/ssm/classlessonscore?id=" +classId+ "&lesson="+subJectName[i]+"&test="+allSubpt_test;
            $.ajax({
                url: "http://www.biggsai.com/ssm/classlessonscore?id=" +classId+ "&lesson="+subJectName[i]+"&test="+allSubpt_test,	//请求url
