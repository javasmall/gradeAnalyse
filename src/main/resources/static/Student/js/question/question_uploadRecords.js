...
function upload_wrongpic_info() {
    ...
    var URL="http://www.biggsai.com/ssm/geterrorcollection?school=just&id="+studentId;
    ...
    $.ajax({
        url: "http://www.biggsai.com/ssm/geterrorcollection?school=just&id="+studentId,//请求url
    ...
    function question_show_Delete(target) {
        var delete_img_head="http://www.biggsai.com/ssm/delete?imgurl=";   //再后面拼接要删除的图片的url,,然后向服务器发送请求即可删除
        ...
        layer.confirm('确认删除本条记录及错题图片吗?', { btn: ['是','否'],
            ...
            $.ajax({
                    url: "http://www.biggsai.com/ssm/delete?imgurl="+delete_imgUrl,
                    type: "GET",
                    // data:{"id":temp},
                     data:" ",
                    success:function () {
