...
var index_student_name="";

'use strict';
layui.use(['jquery','layer','element'],function(){
	window.jQuery = window.$ = layui.jquery;
	window.layer = layui.layer;
  var element = layui.element;  //得到页面的组件element
  
// larry-side-menu向左折叠
$('.larry-side-menu').click(function() {
  var sideWidth = $('#larry-side').width();  //获取左侧侧边导航栏的宽度
  if(sideWidth === 200) {
      $('#larry-body').animate({
        left: '0'
      }); //admin-footer
...
$(function(){
    ...
    if($.cookie('studentId')!=null){   //进行判空操作
        index_student_name=$.cookie('studentId');
        $('#index_student_name').text("账户名:学生 "+index_student_name);
        //console.log("studentID is: "+$.cookie('studentId'));
    }else {
        // $('#index_student_name').text("账户名:学生为空 ");
        $('#index_student_name').text("账户名:学生为10000000141 ");
        // 10000000141
    }
